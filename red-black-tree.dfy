...
static twostate lemma ColorFix(x : RBTreeRef, y : RBTreeRef)
	requires y.parent == old(y.parent)
	requires y.Repr == old(y.Repr)
	requires y.color == old(y.color)
	requires y.ValidWeak()
	requires x.ValidWeak()
	requires y in x.ElemsRef()
	requires old(x.ValidWeak())
	requires old(y.ValidWeak());
	requires old(y in x.ElemsRef())
	requires old(y.PartialNoRR(x))
	requires forall o  :: old(allocated(o)) ==> o !in y.ElemsRef() ==> unchanged(o)


	ensures y.PartialNoRR(x)
 	decreases x.Repr - y.Repr
{

	if(x==y) {
		assert(y.PartialNoRR(x));
		return;
	}

	assert(y.parent != null);
	assert(y.parent !in y.ElemsRef());
	assert(y.parent.ValidWeak());
	assert(unchanged(y.parent));
	assert(y.parent in x.ElemsRef());
	assert(old(y.parent in x.ElemsRef()));
	assert(old(y.parent.PartialNoRR(x)));
	ColorFix(x,y.parent);
				...
				// invariant forall j : nat :: j < index ==> a[j] != key
			{
				...
				if(uncle != null && uncle.color == Red) {
					...
					if(n.parent.parent != r) {
						...
						// assert(n.parent.parent.parent != null);
						// assert(n.parent.parent.parent.ValidWeak());
						// assert(n.parent.parent.ValidWeak());
						// assert(old@L1(n.parent.parent.PartialNoRR(r)));
						// assert(old@L1(n.parent.parent.parent.PartialNoRR(r)));
						// assert(n.parent.parent.parent !in n.parent.parent.ElemsRef());
						// assert(unchanged(n.parent.parent.parent));
						// assert(n.parent.parent.PartialNoRRP());
						// ColorFix(r , n.parent.parent.parent);
